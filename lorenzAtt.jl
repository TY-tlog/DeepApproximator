...
function calculate_lyapunov(t, Y, N)
    ...
    for i in 1:N
        ...
        J = lorenz_jacobian(Y[:,i])
        
        # Evolve vectors using variational equation
        Q = Q + dt * (J * Q)
...
# Solve the ODEs (only need original system, not variational equations)
t, Y = runge_kutta4(lorenz, t0, tf, Y0, N)

# Calculate Lyapunov exponents
lyap_exponents = calculate_lyapunov(t, Y, N)

println("리아푸노프 지수:")
for (i, val) in enumerate(lyap_exponents)
    println("λ$i = $val")
end
